    ...
    <div class="col-md-10">
        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0">Editar Proveedor: {{ $proveedore->razon_social }}</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('proveedores.update', $proveedore) }}" method="POST">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('razon_social') is-invalid @enderror" 
                                       ...
                                       value="{{ old('razon_social', $proveedore->razon_social) }}" 
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('cuit') is-invalid @enderror" 
                                       id="cuit" name="cuit" 
                                       value="{{ old('cuit', $proveedore->cuit) }}" 
                                ...
                                <select class="form-select @error('tipo_servicio_id') is-invalid @enderror" 
                                    ...
                                    @foreach($tiposServicio as $tipo)
                                        <option value="{{ $tipo->id }}" 
                                            {{ old('tipo_servicio_id', $proveedore->tipo_servicio_id) == $tipo->id ? 'selected' : '' }}>
                                            {{ $tipo->nombre }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('estado') is-invalid @enderror" 
                                    ...
                                    <option value="activo" {{ old('estado', $proveedore->estado) == 'activo' ? 'selected' : '' }}>Activo</option>
                                    <option value="inactivo" {{ old('estado', $proveedore->estado) == 'inactivo' ? 'selected' : '' }}>Inactivo</option>
                                </select>
                                @error('estado')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="col-md-12">
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('direccion') is-invalid @enderror" 
                                          ...
                                          rows="3" 
                                          placeholder="Dirección completa del proveedor">{{ old('direccion', $proveedore->direccion) }}</textarea>
                                @error('direccion')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <div id="tipos-componente-container">
                                        @php
                                            $tiposComponenteSeleccionados = old('tipos_componente', $proveedore->tiposComponente->map(function($item) {
                                                return ['tipo_componente_id' => $item->id];
                                            })->toArray());
                                        @endphp
                                        
                                        @if(count($tiposComponenteSeleccionados) > 0)
                                            @foreach($tiposComponenteSeleccionados as $index => $tipoComponente)
                                                <div class="tipo-componente-item border-bottom pb-3 mb-3">
                                                    <div class="row">
                                                        <div class="col-md-11">
                                                            <div class="mb-3">
                                                                <label class="form-label">Tipo de Componente</label>
                                                                <select class="form-select @error('tipos_componente.' . $index . '.tipo_componente_id') is-invalid @enderror" 
                                                                        name="tipos_componente[{{ $index }}][tipo_componente_id]">
                                                                    <option value="">Seleccionar tipo de componente</option>
                                                                    @foreach($tiposComponente as $tipo)
                                                                        <option value="{{ $tipo->id }}" 
                                                                            {{ ($tipoComponente['tipo_componente_id'] ?? '') == $tipo->id ? 'selected' : '' }}>
                                                                            {{ $tipo->nombre }}
                                                                        </option>
                                                                    @endforeach
                                                                </select>
                                                                @error('tipos_componente.' . $index . '.tipo_componente_id')
                                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                                @enderror
                                                            </div>
                                                        </div>
                                            ...
                                            @endforeach
                                        @else
                                            <!-- Primer tipo de componente vacío -->
                                            <div class="tipo-componente-item border-bottom pb-3 mb-3">
                                                <div class="row">
                                                    <div class="col-md-11">
                                                        <div class="mb-3">
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <div id="tipos-equipo-container">
                                        @php
                                            $tiposEquipoSeleccionados = old('tipos_equipo', $proveedore->tiposEquipo->map(function($item) {
                                                return ['tipo_equipo_id' => $item->id];
                                            })->toArray());
                                        @endphp
                                        
                                        @if(count($tiposEquipoSeleccionados) > 0)
                                            @foreach($tiposEquipoSeleccionados as $index => $tipoEquipo)
                                                <div class="tipo-equipo-item border-bottom pb-3 mb-3">
                                                    <div class="row">
                                                        <div class="col-md-11">
                                                            <div class="mb-3">
                                                                <label class="form-label">Tipo de Equipo</label>
                                                                <select class="form-select @error('tipos_equipo.' . $index . '.tipo_equipo_id') is-invalid @enderror" 
                                                                        name="tipos_equipo[{{ $index }}][tipo_equipo_id]">
                                                                    <option value="">Seleccionar tipo de equipo</option>
                                                                    @foreach($tiposEquipo as $tipo)
                                                                        <option value="{{ $tipo->id }}" 
                                                                            {{ ($tipoEquipo['tipo_equipo_id'] ?? '') == $tipo->id ? 'selected' : '' }}>
                                                                            {{ $tipo->nombre }}
                                                                        </option>
                                                                    @endforeach
                                                                </select>
                                                                @error('tipos_equipo.' . $index . '.tipo_equipo_id')
                                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                                @enderror
                                                            </div>
                                                        </div>
                    ...
                    <div class="d-grid gap-2 d-md-flex justify-content-md-end mt-4">
                        <a href="{{ route('proveedores.show', $proveedore) }}" class="btn btn-secondary me-md-2">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <small class="text-muted">
                            <strong>Creación:</strong> {{ $proveedore->created_at->format('d/m/Y H:i') }}
                        </small>
                    </div>
                    <div class="col-md-6">
                        <small class="text-muted">
                            <strong>Última actualización:</strong> {{ $proveedore->updated_at->format('d/m/Y H:i') }}
                        </small>
                    </div>
                </div>
            </div>
...
<script>
// Variables para contar cada tipo - inicializar basado en los elementos existentes
let contadorTiposComponente = {{ count(old('tipos_componente', $proveedore->tiposComponente)) }};
let contadorTiposEquipo = {{ count(old('tipos_equipo', $proveedore->tiposEquipo)) }};

// Si no hay elementos, empezar desde 1
if (contadorTiposComponente === 0) contadorTiposComponente = 1;
if (contadorTiposEquipo === 0) contadorTiposEquipo = 1;
